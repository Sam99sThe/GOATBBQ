import {addCoaster} from './coaster-scene.js';
import {DeliveryScene} from './delivery-scene.js';
...
export class BBQScene {
 constructor({container,labels,onFood,status}){
  ...
  this.nightScenery=addNightScenery(this.scene);this.delivery=new DeliveryScene(this.scene);Promise.all([this.delivery.load,this.delivery.cupLoad]).catch(e=>this.status('飲料機車載入失敗：'+e.message,true));
  ...
  this.clock=new THREE.Clock();this.frame=()=>{if(this.disposed)return;this.animation=requestAnimationFrame(this.frame);if(document.hidden){this.clock.getDelta();return}const dt=Math.min(this.clock.getDelta(),.05);for(const p of this.players.values())animateElephant(p,dt);if(this.preview)animateElephant(this.preview,dt);this.scene.updateMatrixWorld(true);this.delivery.update(this.lastState.delivery,this.players,(this.lastState.now||Date.now())+performance.now()-(this.syncedAt||performance.now()));this.coaster?.update(dt);this.controls.update();this.positionLabels();this.fire.intensity=12+(this.lastState.heat||1)*2+Math.sin(performance.now()*.006);this.renderer.render(this.scene,this.camera)};this.frame();
 }
 ...
 async load(){
  ...
  addCoaster(this.scene,loader,this.elephant).then(c=>this.coaster=c).catch(e=>this.status('雲霄飛車載入失敗：'+e.message,true));
 ...
 sync(state,me){this.syncedAt=performance.now();this.lastState=state;this.me=me;if(!this.ready)return;if(me||state.players.length)this.clearPreview();const ids=new Set(state.players.filter(p=>p.seat!==null).map(p=>p.id));for(const [id,entry] of this.players){if(!ids.has(id)){entry.mixer.stopAllAction();entry.root.removeFromParent();entry.label.remove();this.players.delete(id)}}
 for(const p of state.players.filter(p=>p.seat!==null)){let entry=this.players.get(p.id);if(!entry){entry=this.makeElephant(p.seat);const label=document.createElement('div');label.className='elephant-label';label.innerHTML='<div class="elephant-bubble" hidden></div><span class="elephant-name"></span><div class="elephant-health"></div>';this.labels.append(label);entry.label=label;this.players.set(p.id,entry)}if(p.motion&&p.motion.id!==entry.motionId){entry.motionId=p.motion.id;entry.motion={kind:p.motion.kind,elapsed:Math.max(0,((state.now||Date.now())-p.motion.started)/1000)}}entry.label.classList.toggle('mine',p.id===me?.id);entry.label.querySelector('.elephant-name').textContent=p.name+(p.id===me?.id?' · 你':'');entry.label.querySelector('.elephant-health').textContent=[p.health?.nauseaText,p.health?.angerText].filter(Boolean).join(' / ')||'狀態正常';const bubble=entry.label.querySelector('.elephant-bubble');bubble.textContent=p.bubble||'';bubble.hidden=!p.bubble}
  this.seats.forEach((s,i)=>s.marker.material.opacity=state.players.some(p=>p.seat===i)?.8:.27);
  const foodIds=new Set(state.food.map(f=>f.id));for(const [id,mesh] of this.food)if(!foodIds.has(id)){mesh.userData.label?.remove();mesh.removeFromParent();mesh.material.dispose();this.food.delete(id)}
  for(const f of state.food){let mesh=this.food.get(f.id);if(!mesh){const template=this.foodTemplates[foodModels[f.kind]];if(!template)continue;mesh=new THREE.Mesh(template.geometry,template.material.clone());mesh.scale.setScalar(.53);mesh.position.set((f.slot%3-1)*.67,2.43,(Math.floor(f.slot/3)-1)*.48-.12);mesh.castShadow=true;mesh.receiveShadow=true;mesh.userData.foodId=f.id;const label=document.createElement("span");label.className="food-temperature";this.labels.append(label);mesh.userData.label=label;this.scene.add(mesh);this.food.set(f.id,mesh)}mesh.userData.label.textContent=f.sides.map(v=>Math.round(v)+"%").join(" | ");const burnt=Math.max(...f.sides)>145,ready=Math.min(...f.sides)>=65;mesh.material.color.set(burnt?0x33251b:ready?0xdab688:0xffffff);mesh.rotation.z=f.side?Math.PI:0;mesh.position.y=f.side?2.53:2.43}
 }
 positionLabels(){for(const mesh of this.food.values()){const pos=mesh.position.clone();pos.y+=.38;pos.project(this.camera);const label=mesh.userData.label;label.hidden=pos.z< -1||pos.z>1;label.style.left=(pos.x*.5+.5)*this.container.clientWidth+"px";label.style.top=(-pos.y*.5+.5)*this.container.clientHeight+"px";}const w=this.container.clientWidth,h=this.container.clientHeight;for(const entry of this.players.values()){const head=new THREE.Vector3(.7,2.95,0).applyMatrix4(entry.root.matrixWorld);head.project(this.camera);const visible=head.z>-1&&head.z<1&&Math.abs(head.x)<1.12&&Math.abs(head.y)<1.2;entry.label.hidden=!visible;entry.label.style.left=(head.x*.5+.5)*w+'px';entry.label.style.top=(-head.y*.5+.5)*h+'px'}}
