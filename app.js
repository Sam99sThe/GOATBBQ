import {mountDrawing} from './drawing-ui.js';
...
let avatar=avatars[0],me=null,state=null,events=null,selected=null,toastTimer;
let bbq=null;
...
async function action(data){if(!me){$('#joinDialog').showModal();return}try{const result=await api('action',data);if(result.expelled){handleExpulsion(result.reason);return}}catch(e){toast(e.message)}}
...
const drawing=mountDrawing({api,getMe:()=>me,toast});
function render(){
 renderChat(); bbq?.sync(state,me);renderDrinks();drawing.update(state.game,state.now||Date.now(),state.players);
...
function handleExpulsion(reason){if(!me)return;drawing.close();events?.close();events=null;sessionStorage.removeItem('bbq');me=null;selected=null;$('#foodDialog').close();$('#leave').hidden=true;$('#invite').hidden=true;$('#connection').textContent='渡劫失敗 · 已離席';$('#joinError').textContent=reason;$('#joinDialog').showModal();toast(reason);}
function connect(){events?.close();chatReady=false;events=new EventSource('/api/events?token='+encodeURIComponent(me.token));events.addEventListener('drawing',e=>drawing.strokeUpdate(JSON.parse(e.data)));events.addEventListener('expelled',e=>handleExpulsion(JSON.parse(e.data).reason));events.onmessage=e=>{state=JSON.parse(e.data);$('#connection').textContent='● 炭火連線中';render()};events.onerror=()=>{$('#connection').textContent='正在重新生火…';if(events.readyState===EventSource.CLOSED){events.close();sessionStorage.removeItem('bbq');me=null;$('#joinError').textContent='座位已釋出，請重新入座。';$('#joinDialog').showModal()}}}
$('#joinForm').onsubmit=async e=>{e.preventDefault();const b=e.submitter;b.disabled=true;$('#joinError').textContent='';try{let token;try{token=JSON.parse(sessionStorage.getItem('bbq'))?.token}catch{}me=await api('join',{name:$('#name').value,avatar,room:$('#roomName').value,token});sessionStorage.setItem('bbq',JSON.stringify(me));history.replaceState({},'', '?room='+encodeURIComponent(me.room));$('#joinDialog').close();$('#invite').hidden=false;$('#leave').hidden=false;connect();loadScene()}catch(e){$('#joinError').textContent=e.message;$('#newRoom').hidden=false}finally{b.disabled=false}};
$('#joinDialog').addEventListener('cancel',e=>{if(!me)e.preventDefault()});
let sceneStarted=false;function loadScene(){if(sceneStarted)return;sceneStarted=true;try{
