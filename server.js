...
const csp=`default-src 'self'; script-src 'self' 'sha256-${createHash('sha256').update(importMap).digest('base64')}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self'; frame-src https://www.youtube.com https://www.youtube-nocookie.com; frame-ancestors 'none'`;
...
function snapshot(r){return {id:r.id,now:Date.now(),players:r.players.map(({id,name,avatar,seat,eaten,bubble,bubbleUntil,motion})=>({id,name,avatar,seat,eaten,motion:motion&&Date.now()-motion.started<3400?motion:null,bubble:bubbleUntil>Date.now()?bubble:''})),food:r.food,log:r.log,chat:r.chat,heat:r.heat,event:r.event}}
...
export const server=http.createServer(async(req,res)=>{
 ...
 const files={'/':['index.html','text/html'],'/app.js':['app.js','text/javascript'],'/scene.js':['scene.js','text/javascript'],'/elephant-motion.js':['elephant-motion.js','text/javascript'],'/night-scene.js':['night-scene.js','text/javascript'],'/style.css':['style.css','text/css'],'/credits.json':['credits.json','application/json']};
