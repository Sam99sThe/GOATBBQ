...
import {capacityConfig,bucket,safeSend} from './load-guard.js';
import {readFile,stat} from 'node:fs/promises';
import {createReadStream} from 'node:fs';
import {randomUUID,createHash} from 'node:crypto';
import {resolve,extname,sep} from 'node:path';
import {fileURLToPath} from 'node:url';
import {applyMeal,health} from './game-rules.js';
import {orderDrink,advanceDrinks} from './drinks.js';
import {gameAction,gameView,tickGame,leaveGame} from './drawing-game.js';
import {openExpulsionStore} from './expulsion-store.js';
const store=(process.env.RENDER||process.env.NODE_ENV==='production')?null:openExpulsionStore(process.env.BBQ_DB_PATH);
const capacity=capacityConfig(),allowApi=bucket(160,240),allowJoin=bucket(2,30);let transfers=0;
...
const csp=`default-src 'self'; script-src 'self' 'sha256-${createHash('sha256').update(importMap).digest('base64')}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self' blob:; frame-ancestors 'none'`;
...
function snapshot(r,viewer,includeCanvas=false){return {game:gameView(r.game,viewer,includeCanvas),delivery:r.delivery||null,id:r.id,now:Date.now(),players:r.players.map(({id,name,avatar,seat,eaten,bubble,bubbleUntil,motion,nausea,anger})=>({id,name,avatar,seat,eaten,health:health({nausea,anger}),motion:motion&&Date.now()-motion.started<3400?motion:null,bubble:bubbleUntil>Date.now()?bubble:''})),food:r.food,log:r.log,chat:r.chat,heat:r.heat,event:r.event}}
function publish(r){for(const p of r.players)if(p.stream&&!p.stream.destroyed)safeSend(p.stream,'data: '+JSON.stringify(snapshot(r,p.id))+'\n\n')}
function remove(p,reason){const r=rooms.get(p.room);if(reason&&p.stream&&!p.stream.destroyed)p.stream.write('event: expelled\ndata: '+JSON.stringify({reason})+'\n\n');p.stream?.end();sessions.delete(p.token);if(!r)return;leaveGame(r.game,p.id);r.players=r.players.filter(x=>x!==p);if(p.seat!==null){const next=r.players.find(x=>x.seat===null);if(next){next.seat=p.seat;log(r,`${next.name} 排到了！大象正式入座。`)}}log(r,reason||`${p.name} 帶著一身不存在的炭味離席。`);if(!r.players.length)rooms.delete(r.id);else publish(r)}
...
export const server=http.createServer(async(req,res)=>{
 ...
 if(!allowApi()){res.setHeader('Retry-After','2');fail('烤爐有點忙，請稍候再試。',429)}
 if(req.headers.origin&&new URL(req.headers.origin).host!==req.headers.host)fail('不允許跨站操作',403);
 let raw='';for await(const part of req){raw+=part;if(raw.length>4096)fail('內容太長',413)}let b;try{b=JSON.parse(raw)}catch{fail('無效內容')}if(!b||typeof b!=='object'||Array.isArray(b))fail('無效內容');
 ...
 const roomName=typeof b.room==='string'?b.room.normalize('NFC').trim():'';if(roomName&&!/^(?=.*[A-Za-z])(?=.*[0-9])[A-Za-z0-9]{5,10}$/.test(roomName))fail('房間名稱需 5～10 碼，且同時包含英文字母與數字。');if(!allowJoin()){res.setHeader('Retry-After','15');fail('入場的人有點多，請等 15 秒再試。',429)}if(sessions.size>=capacity.players){res.setHeader('Retry-After','30');fail('烤肉場客滿，請稍後再來；現有玩家會繼續遊玩。',503)}let r=roomName?rooms.get(roomName):[...rooms.values()].find(r=>r.players.length<5);if(!r){if(rooms.size>=capacity.rooms)fail('目前烤爐已全數開桌，請稍後再試或加入朋友的房間。',503);r=makeRoom(roomName);}if(r.players.length>=15)fail('這房已有 5 隻大象與 10 位卡位者，請換一個房間。',409);
 
 const seat=[0,1,2,3,4].find(s=>!r.players.some(p=>p.seat===s))??null;const p={id:randomUUID(),token:randomUUID(),name,avatar:avatars.includes(b.avatar)?b.avatar:avatars[0],seat,room:r.id,eaten:0,nausea:0,anger:0,last:Date.now(),action:0};r.players.push(p);sessions.set(p.token,p);log(r,seat===null?`${name} 加入卡位隊伍，可以先聊天偷吃。`:`${name} 坐下了。現實中的椅子並沒有動。`);publish(r);return json(200,{token:p.token,id:p.id,room:r.id})}
 ...
 if(url.pathname==='/api/game'){if(Date.now()-(p.gameActionAt||0)<25)fail('畫筆稍微慢一點。',429);p.gameActionAt=Date.now();gameAction(r,p,b);if(b.type==='draw'){const g=r.game,data='event: drawing\ndata: '+JSON.stringify({canvasKey:g.id+':'+g.round+':'+g.epoch,version:g.version,stroke:g.strokes.at(-1)})+'\n\n';for(const viewer of r.players)safeSend(viewer.stream,data);}else publish(r);return json(200,{ok:true})}
 ...
 if(req.method==='GET'&&url.pathname==='/api/events'){const p=sessions.get(url.searchParams.get('token'));if(!p)return json(401,{error:'座位已釋出'});p.stream?.end();res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache, no-transform','Connection':'keep-alive','X-Accel-Buffering':'no'});res.write(': connected\n\n');p.stream=res;p.last=Date.now();safeSend(res,'data: '+JSON.stringify(snapshot(rooms.get(p.room),p.id,true))+'\n\n');req.on('close',()=>{if(p.stream===res){p.stream=null;p.last=Date.now()}});return}
 const files={'/coaster-scene.js':['coaster-scene.js','text/javascript'],'/privacy':['privacy.html','text/html'],'/third-party-notices.txt':['THIRD_PARTY_NOTICES.txt','text/plain'],'/drawing-ui.js':['drawing-ui.js','text/javascript'],'/delivery-scene.js':['delivery-scene.js','text/javascript'],'/drinks.js':['drinks.js','text/javascript'],'/':['index.html','text/html'],'/app.js':['app.js','text/javascript'],'/scene.js':['scene.js','text/javascript'],'/elephant-motion.js':['elephant-motion.js','text/javascript'],'/night-scene.js':['night-scene.js','text/javascript'],'/style.css':['style.css','text/css'],'/credits.json':['credits.json','application/json']};
 ...
 if(transfers>=capacity.transfers){res.setHeader('Retry-After','3');return json(503,{error:'模型搬運中，請稍後重新整理。'})}transfers++;res.once('close',()=>{transfers--;});
 res.writeHead(200,{'Content-Type':type,'Content-Length':info.size,'Cache-Control':'public, max-age=3600','X-Content-Type-Options':'nosniff'});const stream=createReadStream(path);stream.on('error',()=>res.destroy());res.once('close',()=>stream.destroy());stream.pipe(res);return;
 }
...
const timer=setInterval(()=>{for(const r of rooms.values()){for(const p of [...r.players]){if(p.stream)p.last=Date.now();else if(Date.now()-p.last>45000)remove(p)}if(!rooms.has(r.id))continue;advanceDrinks(r);tickGame(r.game);for(const f of r.food)f.sides[f.side]=Math.min(200,f.sides[f.side]+100/foods[f.kind][1]*r.heat);r.heat=Math.max(1,r.heat-.04);if(Date.now()>r.nextEvent){const events=['🛸 外星人路過：給這攤一顆米其林隕石。','🌬️ 風把香味吹進了隔壁的試算表。','📞 媽媽來電：你在電腦前烤什麼肉？','🦆 鴨子衛生局：准許邊烤邊胡說八道。'];r.event=events[Math.floor(Math.random()*events.length)];log(r,r.event);r.nextEvent=Date.now()+30000}publish(r)}},1000);timer.unref();server.maxConnections=256;server.headersTimeout=15000;server.requestTimeout=15000;server.keepAliveTimeout=5000;server.on('close',()=>{clearInterval(timer);store?.close();});
