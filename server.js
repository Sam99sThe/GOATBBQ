...
function makeRoom(){let id;do{id=randomUUID().slice(0,6).toUpperCase()}while(rooms.has(id));const r={id,players:[],food:[],log:[],heat:1,event:'今晚的熱量，只存在於想像。',nextEvent:Date.now()+30000};rooms.set(id,r);return r}
function snapshot(r){return {id:r.id,players:r.players.map(({id,name,avatar,seat,eaten,bubble,bubbleUntil})=>({id,name,avatar,seat,eaten,bubble:bubbleUntil>Date.now()?bubble:''})),food:r.food,log:r.log,heat:r.heat,event:r.event}}
